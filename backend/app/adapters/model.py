...
class ModelAdapter:
    ...
    async def route_request(
        ...
        system = (
            ...
            "entities, filters and evidence needs. Do not route by isolated keywords. "
            "Classify the request as general, knowledge_query, business_query, composite, "
            "action or clarify. Current business facts such as inventory, stock, sales, "
            "production, procurement, orders, amounts and statuses must use business data. "
            "Do not use document search as a substitute for current business records. "
            "Policies, procedures, definitions and how-to explanations use enterprise "
            "knowledge. Composite questions query business facts first and then supporting "
            "knowledge evidence. Write, delete, approve and other state-changing actions "
            "are action and must not be executed by read-only tools.\n"
            "\u7981\u6b62\u7528\u5355\u4e2a\u5173\u952e\u8bcd\u51b3\u5b9a\u8def\u7531\uff1b\u4e0d\u80fd\u770b\u5230\u52a8\u4f5c\u8bcd\u5c31\u76f4\u63a5\u62d2\u7edd\u3002"
            "\u7edd\u4e0d\u80fd\u7528 knowledge.search \u6216\u6587\u6863\u7247\u6bb5\u4ee3\u66ff\u5f53\u524d\u4e1a\u52a1\u4e8b\u5b9e\uff1b"
            "\u5de5\u5177\u987a\u5e8f\u5fc5\u987b\u5148\u4e1a\u52a1\u6570\u636e\u540e\u77e5\u8bc6\u8bc1\u636e\u3002\n"
            "Business data is not limited to a fixed list of procurement Tools. If a "
            "fixed domain-specific Tool is not present, choose data.business.query for an "
            "authorized read-only semantic query. It accepts a logical dataset_id such as "
            "inventory, stock, sales or production, never raw SQL. Put the logical subject "
            "in tool_arguments.dataset_id when possible; if it is omitted, the executor "
            "can derive it from entity or domain. Do not mark a capability unavailable only "
            "because there is no same-named Tool. The gateway resolves published semantic "
            "datasets or discovers matching tables only on approved read-only connectors.\n"
            "All current enterprise business data, including procurement, inventory, sales "
            "and production, must use data.business.query. There are no model-facing fixed "
            "procurement Tools. Procurement-specific formatting is implemented by the domain "
            "extension after the universal Tool returns a DataArtifact. For a single order, "
            "require order_number and clarify if missing. "
            "Never fabricate Tool ids, switch a data question to documents, or invent facts. "
            "Output JSON only with request_kind, domain, operation, entity, identifiers, "
            "filters, data_needs, evidence_need, confidence, required_tools, "
            "tool_arguments, missing_fields, clarification_question, capability_available, "
            "unavailable_capability and summary. identifiers, filters and tool_arguments "
            "must be JSON objects; data_needs may only contain public_knowledge, "
            "enterprise_knowledge and business_data.\n"
        )
        user = (
